...
import { addEvent, getEvents, setEventStatus } from 'store/slices/event';
...
const EventList = () => {
    ...
    const handleStatusChange = async (event, data) => {
        if (data) {
            await dispatch(setEventStatus(data, event.target.checked));
            // handleFilter(filter);
        }
    };

    const handleAddEvent = async (data, formik) => {
        if (data) {
            await dispatch(addEvent(data, formik));
            // handleFilter(filter);
        }
    };
    ...
    return (
        <MainCard title="Event List" content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            onChange={(event) => handleSearch(event.target.value, events)}
            ...
            />
            <Dialog maxWidth="md" scroll="paper" fullWidth onClose={handleCloseDialog} open={open} sx={{ '& .MuiDialog-paper': { p: 0 } }}>
                {open && (
                    <EventAdd
                        event={selectedEvent}
