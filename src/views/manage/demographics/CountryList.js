...
import { getCountries, setCountryStatus } from 'store/slices/demographics';
...
const CountryList = () => {
    ...
    const handleSearch = (value, countries) => {
        const newString = value;
        setSearch(newString || '');

        if (newString) {
            const newRows = countries.filter((row) => {
                let matches = true;
    ...
    const handleStatusChange = async (event, data) => {
        if (data) {
            await dispatch(setCountryStatus(data, event.target.checked));
            // handleFilter(filter);
        }
    };

    return (
        <MainCard title="Country List" content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            onChange={(event) => handleSearch(event.target.value, countries)}
