...
import { getCountries, getStatesLocal, setStateStatus } from 'store/slices/demographics';
...
const StateList = () => {
    ...
    const staticCountries = useSelector((state) => state.demographics.countries);
    ...
    const handleStatusChange = async (event, data) => {
        if (data) {
            await dispatch(setStateStatus(data, event.target.checked));
            // handleFilter(filter);
        }
    };

    return (
        <MainCard title="State List" content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        <TextField
                            ...
                            onChange={(event) => handleSearch(event.target.value, states)}
